 ...
 * rclcpp/rclcpp.hpp include which allows you to use the most common pieces of
 * the ROS 2 system
 * std_msgs/msg/string.hpp, which includes the built-in message type you will
 * use to publish data.
 * serverpublisher/msg/num.hpp includes the custom built msg types in the
 * tutorial interfaces.
 *
 * */
...
public:
  ...
  /* The public constructor names the node minimal_publisher and initializes
     count_ to 0.
   * inside the constructor, the publisher is initialized with the custom int
   * message type,
   * the topic name Life iteration, and the required queue size to limit
   * messages in the event of a backup.
   * Next, timer_ is initialized, which causes the timer_callback function to be
  ...
  MinimalPublisher() : Node("minimal_publisher"), count_(0)
  {
    ...
    try {
      ...
      auto param_callback_ptr = std::bind(&MinimalPublisher::param_callback,
                                          this,
                                          _1);
      param_handle_ = param_subscriber_->add_parameter_callback("frequency",
                                                                param_callback_ptr);
      ...
      timer_ =
        this->create_wall_timer(500ms,
                                std::bind(&MinimalPublisher::timer_callback,
                                          this));
    } catch (...) {
...
private:
  ...
  /* The timer_callback function is where the message data is set and the
     messages are actually published.
   * The RCLCPP_INFO macro ensures every published message is printed to the
   * console.
   * */
  void timer_callback()
  {
