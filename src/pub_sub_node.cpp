...
class PublisherSubscriber : public rclcpp::Node {
 public:
  ...
  PublisherSubscriber() : Node("publishing_subscribing") {
    ...
    publisher_ = this->create_publisher<serverpublisher::msg::Num>("No_Life_iteration", 10);
  }
 ...
 private:
  ...
  void topic_callback(const serverpublisher::msg::Num & msg) const {
    ...
    if (msg.a > 30) {
      RCLCPP_INFO_STREAM(
        rclcpp::get_logger("rclcpp"), "Calling client for requesting addition of three numbers");
      Set_Client(msg);
  ...
  void Set_Client(const serverpublisher::msg::Num & msg) const {
    ...
    auto request = std::make_shared<serverpublisher::srv::AddThreeInts::Request>();
    request->a = msg.a;
    request->b = msg.b;
    request->c = msg.c;
    ...
    if (rclcpp::spin_until_future_complete(node, result) == rclcpp::FutureReturnCode::SUCCESS) {
      ...
      RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("rclcpp"), "Failed to call service add_three_ints");  // Error Log
    }
  }
